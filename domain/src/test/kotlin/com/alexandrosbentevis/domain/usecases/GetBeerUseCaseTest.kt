...
class GetBeerUseCaseTest {
    ...
    private lateinit var useCase: GetBeerUseCase
    ...
    fun `When the use case is invoked, then the repository should be called`() {
        coroutinesTestRule.testDispatcher.runBlockingTest {
            ...
            val params = GetBeerUseCase.Params(id = beer.id)

            beerRepository = mock {
                onBlocking { getBeerById(any(), any()) } doReturn flow { emit(Result.Success(beer)) }
            }

            useCase = GetBeerUseCase(beerRepository = beerRepository)

            // WHEN
            useCase(params)

            // THEN
            verify(beerRepository).getBeerById(id = params.id)
        }
    }

    @Test
    fun `Given the repository returns a beer wrapped in a success result, when the use case is invoked, then the success result should be returned`() {
        coroutinesTestRule.testDispatcher.runBlockingTest {
            // GIVEN
            val beer = BeerFactory.createBeer()
            val params = GetBeerUseCase.Params(id = beer.id)
            ...
            data `should equal` Result.Success(beer)
        }
    }
    ...
    fun `Given the repository returns an error result, when the use case is invoked, then the error result should be returned`() {
        coroutinesTestRule.testDispatcher.runBlockingTest {
            ...
            val params = GetBeerUseCase.Params(id = "12")

            beerRepository = mock {
                onBlocking { getBeerById(any(), any()) } doReturn flow { emit(Result.Error(exception)) }
            }
